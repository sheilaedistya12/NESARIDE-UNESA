...
# Fungsi untuk mencari driver terdekat menggunakan KNN
def find_nearest_driver(user_lat, user_lon, drivers):
    # Memasukkan koordinat driver ke dalam array
    driver_coords = drivers[['Latitude', 'Longitude']].values

    # Membuat model KNN
    knn = NearestNeighbors(n_neighbors=1, metric='haversine')
    
    # Mengonversi koordinat pengguna ke dalam radian
    user_coords = np.radians([[user_lat, user_lon]])

    # Melatih model dengan data koordinat driver
    knn.fit(np.radians(driver_coords))

    # Mencari driver terdekat
    distances, indices = knn.kneighbors(user_coords)
    nearest_driver = drivers.iloc[indices[0][0]]
    min_distance = distances[0][0] * 6371  # Mengonversi jarak ke kilometer
